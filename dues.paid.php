<?php 
	...
	if (isset($_SESSION['member.paid'])) {
		$lvl = isset($_GET['level']) ? $_GET['level'] : '';
		$reference  = sanitize($_SESSION['member.paid']);
		$sql = "
			SELECT * FROM tein_dues 
            INNER JOIN tein_membership 
            ON tein_dues.member_id = tein_membership.id 
		";
		if ($lvl == 'L100') {
            $sql .= "
                WHERE tein_dues.level_100 = ? 
            ";
        } elseif ($lvl == 'L200') {
            $sql .= "
                WHERE tein_dues.level_200 = ? 
            ";
        } elseif ($lvl == 'L300') {
            $sql .= "
                WHERE tein_dues.level_300 = ? 
            ";
        } elseif ($lvl == 'L400') {
            $sql .= "
                WHERE tein_dues.level_400 = ? 
            ";
        }
        $sql .= ' LIMIT 1';
...
<body>
    ...
    <div class="dropdown position-fixed bottom-0 end-0 mb-3 me-3 bd-mode-toggle">
        <button class="btn btn-success py-2 dropdown-toggle d-flex align-items-center"
              id="bd-theme"
              type="button"
              aria-expanded="false"
	...
	<div class="container my-5">
	  	<div class="position-relative p-5 text-center text-muted bg-body border border-dashed rounded-5">
	    	...
	    	<p class="col-lg-6 mx-auto mb-4">
	      		Dues paid is useful for placeholder content. It's also a great way to add a bit of context to a page or section when no content is available and to encourage visitors to take a specific action.
	    	</p>
	...
	<?php
			...
			$_SESSION['member.paid'] = 'tein';
			// session_destroy($_SESSION['member']);
			redirect(PROOT);
		}
	} else {
		$_SESSION['member.paid'] = 'tein';
